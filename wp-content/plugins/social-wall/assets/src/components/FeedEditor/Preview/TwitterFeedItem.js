...
const TwitterFeedItem = ( { post, feedSettings, pluginName } ) => {
        ...
        <>
            <div className="sbsw-follow">
                <a href={post?.account_link} target="_blank" rel="nofollow noopener">
                    <>
                        {SbUtils.printIcon( 'twitter-logo', 'follow-icon' ) }
                        <span>Subscribe</span>
                    </>
                </a>
            </div>
            <div className="sbsw-post-item-inner">
                <div className="sbsw-item-header sbsw-no-avatar">
                    <div className="sbsw-identity">
                                ...
                                { 
                                    feedSettings?.postElements.includes('date') && (
                                        <div className="sbsw-date">
                                            { feedSettings?.dateBeforeText + ' ' + SbUtils.printDate(toTimestamp(post?.created_at), feedSettings ) + ' ' + feedSettings?.dateAfterText}
                                        </div>
                                    )
                                }
                            </div>
                        ...
                        <div className="sbsw-icon">
                            <a href={post.permalink}>
                                { SbUtils.printIcon( 'twitter-logo' ) }
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div className="sbsw-item-footer">
                    <div className="sbsw-item-bottom">
                        { 
                            feedSettings?.postElements.includes('summary') && (
                                <div className="sbsw-item-stats">
                                    <div className="sbsw-item-likes">
                                        { SbUtils.printIcon( 'retweet' ) }
                                        <span className='sbsw-summary-text'>{post.stats_data.retweet_count ? post.stats_data.retweet_count : '0'}</span>
                                    </div>
                                    <div className="sbsw-item-comments">
                                        { SbUtils.printIcon( 'comment' ) }
                                        <span className='sbsw-summary-text'>{post.stats_data.favourite_count ? post.stats_data.favourite_count : '0'}</span>
                                    </div>
                                </div>
                            )
                        }
