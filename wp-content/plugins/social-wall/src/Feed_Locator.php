...
class Feed_Locator
{
	...
	public function retrieve_matching_entries() {
		global $wpdb;
		$feed_locator_table_name = esc_sql( $wpdb->prefix . 'sw_feed_locator' );
	...
	public function add_or_update_entry() {
		...
		if ( empty( $this->matching_entries ) ) {
			...
			foreach ( $this->matching_entries as $index => $matching_entry ) {
				...
				$diff = array_diff(array_map('json_encode', $matching_atts), array_map('json_encode', $details_atts));
				$atts_diff = array_map('json_decode', $diff); // determines if the shortcode settings match the shortcode settings of an existing feed
	...
	public static function should_clear_old_locations() {
		$sbsw_statuses_option = get_option( 'sbsw_statuses', array() );
		$last_old_feed_check = isset( $sbsw_statuses_option['feed_locator']['last_check'] ) ? $sbsw_statuses_option['feed_locator']['last_check'] : 0;
	...
	public static function delete_old_locations() {
		...
		$sbsw_statuses_option = get_option( 'sbsw_statuses', array() );
		$sbsw_statuses_option['feed_locator']['last_check'] = time();
		if ( ! isset( $sbsw_statuses_option['feed_locator']['initialized'] ) ) {
			$sbsw_statuses_option['feed_locator']['initialized'] = time();
		}

		update_option( 'sbsw_statuses', $sbsw_statuses_option, true );
	}
	...
	public static function should_do_locating() {
		$sbsw_statuses_option = get_option( 'sby_statuses', array() );
		if ( isset( $sbsw_statuses_option['feed_locator']['initialized'] )
		     && $sbsw_statuses_option['feed_locator']['initialized'] < (time() - 300) ) {
	...
	public static function legacy_feed_locator_query( $args ) {
		global $wpdb;
		$feed_locator_table_name = $wpdb->prefix . 'sw_feed_locator';
		...
		$offset = max( 0, $page * Database::RESULTS_PER_PAGE );
		$limit  = Database::RESULTS_PER_PAGE;
