...
class SB_Customview_Control extends SB_Controls_Base{
	...
	*/
	...
	<div class="sb-control-feedtype-ctn" v-if="control.viewId == 'sources'">
		...
		<div class="sb-control-feedtype-item sbsw-fs" v-for="(feedType, feedTypeID) in selectSourceScreen.multipleTypes" v-if="checkMultipleFeedTypeActiveCustomizer(feedTypeID)">
			<div class="sb-control-elem-label-title sbsw-fs">
			...
			<div class="sb-control-feedtype-list sbsw-fs" v-if="feedTypeID != 'mentionstimeline' && feedTypeID != 'hometimeline'">
		...
		<button class="sb-control-action-button sb-btn sb-btn-grey sbsw-fs" @click.prevent.default="openFeedTypesPopupCustomizer()">
	...
	?>
		<div :class="['sb-control-feedtype-ctn sb-control-feedtemplate-ctn', 'ctf-feedtemplate-' + customizerScreens.printedTemplate.type]" v-if="control.viewId == 'feedtemplate'">
			<div class="ctf-fb-type-el" v-if="customizerFeedTemplatePrint()"  @click.prevent.default="activateView('feedtemplatesPopup')">
				<div class="ctf-fb-type-el-img sbsw-fs" v-html="svgIcons[customizerScreens.printedTemplate.icon]"></div>
				<div class="ctf-fb-type-el-info sbsw-fs">
					<strong class="sbsw-fs" v-html="customizerScreens.printedTemplate.title"></strong>
				</div>
			</div>
			<button class="sb-control-action-button sb-btn sbsw-fs sb-btn-grey" @click.prevent.default="activateView('feedtemplatesPopup')">
				<div v-html="svgIcons['edit']"></div>
				<span>{{genericText.change}}</span>
			</button>
			<p class="ctf-fb-feedtemplate-alert sbsw-fs">
