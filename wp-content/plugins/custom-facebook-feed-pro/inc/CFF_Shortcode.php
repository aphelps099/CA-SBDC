							...
							if ($cff_offset_show_post) {
								...
								if (!$cff_photos_only && !$cff_videos_only && !($cff_albums_only && $cff_albums_source == 'photospage') && empty($cff_album_id) && !$cff_reviews) {
									...
									if ($news->type == 'photo' || $news->type == 'album' || $news->type == 'offer' || ( $cff_featured_post_active && !empty($this->feed_options['featuredpost']) && isset($news->images) )) {
										...
										$cff_media .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" data-orig-source="' . $picture . '" alt="' . htmlspecialchars($cff_alt_text) . '" data-querystring="' . $cff_picture_querystring . '" data-ratio="' . round($cff_main_img_width / $cff_main_img_height, 3) . '" class="cff-multi-image cff-feed-image" />';
										if ($cff_img_count > 1) {
											$cff_media .= '</span>';
										}
									...
									if ($cff_post_type == 'video' && !$cff_soundcloud && !$cff_spotify) {
										...
										// Check whether it's a vimeo
											...
											if (!empty($news->source)) {
												...
												$cff_media_video .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" alt="Video image" class="cff-iframe-img cff-feed-image" />';
