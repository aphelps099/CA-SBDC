...
class EDD_SL_Plugin_Updater
{
	...
	private function get_version_from_remote()
	{
		...
		if ($request && isset($request->sections)) {
			$request->sections = unserialize($request->sections, ['allowed_classes' => false]);
		} else {
			$request = false;
		}

		if ($request && isset($request->banners)) {
			$request->banners = unserialize($request->banners, ['allowed_classes' => false]);
		}

		if ($request && isset($request->icons)) {
			$request->icons = unserialize($request->icons, ['allowed_classes' => false]);
		}
