...
class Feedzy_Rss_Feeds_Pro_Wordai implements Feedzy_Rss_Feeds_Pro_Services_Interface {
	...
	public function check_api( &$post_data, $settings ) {
		if ( isset( $post_data['wordai_pass'] ) && $post_data['wordai_pass'] !== '' ) {
			if ( isset( $settings['wordai_hash'] ) && $settings['wordai_hash'] === md5( substr( md5( $post_data['wordai_pass'] ), 0, 15 ) ) ) {
				$post_data['wordai_hash'] = $settings['wordai_hash'];
				unset( $post_data['wordai_pass'] );
			} else {
				$post_data['wordai_hash'] = md5( substr( md5( $post_data['wordai_pass'] ), 0, 15 ) );
				unset( $post_data['wordai_pass'] );
			}
		}
		...
		$response = wp_remote_post(
			'http://wordai.com/users/account-api.php',
			array(
				'method' => 'POST',
				'timeout' => 120,
				'redirection' => 5,
	...
	public function call_api( $settings, $text, $type, $additional = array() ) {
		...
		$args       = apply_filters(
			'feedzy_wordai_args', array_merge(
				$additional, array(
					'quality' => 'Regular',
					'sentence' => 'on',
					'returnspin' => 'true',
		...
		$response = wp_remote_post(
			...
			apply_filters(
				'feedzy_service_api_params', array(
					'method' => 'POST',
					'timeout' => 120,
					'redirection' => 5,
					'httpversion' => '1.0',
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( isset( $decode_response['error'] ) && $decode_response['error'] !== '' ) {
				...
				$spun_text  = $decode_response['text'];
				if ( 'title' !== $type ) {
