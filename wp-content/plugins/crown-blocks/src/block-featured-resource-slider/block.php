...
if(!class_exists('Crown_Block_Featured_Resource_Slider')) {
	class Crown_Block_Featured_Resource_Slider extends Crown_Block {


		public static $name = 'featured-resource-slider';
		...
		public static function init() {
			...
			add_filter( 'crown_blocks_prev_resource_ids', array( get_called_class(), 'filter_crown_blocks_prev_resource_ids' ) );
		...
		public static function get_attributes() {
			return array(
				...
				'filterTypes' => array( 'type' => 'array', 'default' => array(), 'items' => array( 'type' => 'object' ) ),
		...
		public static function render( $atts, $content ) {
			...
			if ( boolval( $atts['excludePrevPosts'] ) ) {
				$prev_post_ids = apply_filters( 'crown_blocks_prev_resource_ids', array() );
				$queryArgs['post__not_in'] = array_unique( array_merge( $queryArgs['post__not_in'], $prev_post_ids ) );
			}

			if ( ! empty( $atts['filterTypes'] ) ) {
				$queryArgs['tax_query'][] = array( 'taxonomy' => 'resource_type', 'terms' => array_map( function( $n ) { return $n['id']; }, $atts['filterTypes'] ) );
			}
			...
			$block_class = array( 'wp-block-crown-blocks-featured-resource-slider' );
						...
						<div class="post-feed item-count-<?php echo $query->post_count; ?>" data-item-count="<?php echo $query->post_count; ?>">
							<div class="inner">
								...
								<?php while ( $query->have_posts() ) { ?>
									<?php $query->the_post(); ?>
									<?php Crown_Resources::resource_teaser(); ?>
								<?php } ?>
		...
		public static function filter_crown_blocks_prev_resource_ids( $prev_post_ids = array() ) {
			return array_unique( array_merge( $prev_post_ids, self::get_output_post_ids() ) );
		}


	}
	Crown_Block_Featured_Resource_Slider::init();
}