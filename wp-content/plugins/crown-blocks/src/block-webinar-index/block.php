...
if(!class_exists('Crown_Block_Webinar_Index')) {
	class Crown_Block_Webinar_Index extends Crown_Block {


		public static $name = 'webinar-index';
		...
		public static function render( $atts, $content ) {
			...
			$filters = (object) array(
				'topic' => (object) array( 'key' => 'w_topic', 'queried' => null, 'options' => array() ),
				'search' => (object) array( 'key' => 'w_search', 'queried' => null, 'options' => array() )
			);
			...
			$block_class = array( 'wp-block-crown-blocks-webinar-index', 'post-feed-block', $atts['className'] );
			$block_id = 'post-feed-block-' . md5( json_encode( array( 'webinar-index', $atts ) ) );
								...
								<div class="post-feed item-count-<?php echo $query->post_count; ?>" data-item-count="<?php echo $query->post_count; ?>">
									<div class="inner infinite-loader-container">
										...
										<?php while ( $query->have_posts() ) { ?>
											<?php $query->the_post(); ?>
											<?php if ( class_exists( 'Crown_Webinars' ) ) Crown_Webinars::webinar_teaser(); ?>
										<?php } ?>
	...
	Crown_Block_Webinar_Index::init();
}