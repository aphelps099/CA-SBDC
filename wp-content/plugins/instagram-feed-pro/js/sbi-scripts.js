...
//                  console.log('not raising res for replacement  ' + currentUrl, reason);
      ...
      removeExtraCarouselPages: function(num) {
        ...
        if (this.layout === 'carousel') {
          $self.find('.sbi-owl-item').slice(num - $self.find('.sbi_item').length).remove();
          setTimeout(function() {
            $self.find('.sbi_carousel').trigger("destroy.owl.carousel");
            $self.find('.sbi_carousel').sbiOwlCarousel(this.carouselArgs);
          }, 0);
        } else {
      ...
      hideExtraImagesForWidth: function() {
        ...
        console.log($self.find('.sbi_item').length)
        if ($(window).width() < 480) {
          if (nummobile < $self.find('.sbi_item').length) {
            this.removeExtraCarouselPages(nummobile);
          }
        } else if (num < $self.find('.sbi_item').length) {
            this.removeExtraCarouselPages(num);
        } else {
            $self.find('.sbi_num_diff_hide').removeClass('sbi_num_diff_hide');
        }
      },
