...
class GP_Google_Sheets_Legacy_Tokens {
	public static function get_global_token() {
		$settings = GP_Google_Sheets::get_instance()->get_plugin_settings();
	...
	public static function delete_global_token() {
		$settings = GP_Google_Sheets::get_instance()->get_plugin_settings();
		unset( $settings['token'] );
		GP_Google_Sheets::get_instance()->update_plugin_settings( $settings );
	}

	public static function delete_feed_token( $feed ) {
		$feed_addon            = GP_Google_Sheets::get_instance();
		$feed['meta']['token'] = null;
		$feed_addon->update_feed_meta( $feed['id'], $feed['meta'] );
	}

	public static function get_feed_access_token( $feed ) {
		// if token is in the feed meta and generated with Gwiz OAuth, then migrate and return it.
		$token = rgars( $feed, 'meta/picked_token' );
		if ( $token ) {
			return self::maybe_refresh_token( $token, $feed );
		}

		$token = self::get_global_token();
		if ( $token ) {
			return self::maybe_refresh_token( $token, $feed, true );
		}
