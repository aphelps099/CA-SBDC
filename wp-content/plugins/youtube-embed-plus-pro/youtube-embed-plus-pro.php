...
//        }
    ...
    public static function get_video_details($video_id_list, $api_parts)
    {
        try
        {
            ...
            if (!is_wp_error($api_response))
            {
                ...
                if (!empty($raw))
                {
                    ...
                    if (!isset($json['error']) && is_array($json) && count($json['items']))
                    {
                        ...
                        $daysecs = 3 * 60 * 60;
                        set_transient($cache_key, $items, $daysecs);
                        $allk = get_option(self::$spdcall, array());
                        $allk[] = $cache_key;
                        update_option(self::$spdcall, $allk);
                    ...
                    {
                        return null; // . ' <em>(Error code ' . $json['error']->code . ': ' . $json['error']->message . ')</em>';
                    }
                }
            }
        }
    ...
    public static function get_channel_streams($channel_id, $event_type)
    {        
        try
        {
                ...
                if (!empty($raw))
                {
                    ...
                    if (!isset($json['error']) && is_array($json))
                    {
                        ...
                        if (count($items))
                        {
                            ...
                            {
                                $item_ids[] = $items[$i]['id']['videoId'];
                            }                        
                            $items_details = self::get_video_details($item_ids, 'snippet,liveStreamingDetails');
                            usort($items_details, array(get_class(), 'compare_scheduled_date'));
                        }
                        $daysecs = 12 * 60 * 60;
                        set_transient($cache_key, $items_details, $daysecs);
                        $allk = get_option(self::$spdcall, array());
                        $allk[] = $cache_key;
                        update_option(self::$spdcall, $allk);
    ...
    public static function get_html($m, $iscontent, $isoverride)
    {
        ...
        if (stripos($linkparamstemp[0], 'live_stream') !== false)
        {
            ...
            if (!empty($channel_streams_now) && $channel_streams_now != 'EMPTY')
            {
            ...
            if (!empty($channel_streams_upcoming) && $channel_streams_upcoming != 'EMPTY')
            {
                //$now_utc = str_replace('+00:00', 'Z', date('c'));
            ...
            {
                return '<div class="epyt-video-wrapper epyt-do-live-fallback"></div>';
            }
            else
            {
                $linkparams['live'] = '1';
        ...
        else
        {
            ...
            $code_iframe2 = '" class="__youtube_prefs__ ' . (!empty($finalparams['live_stream']) || !empty($finalparams['streams']) ? ' epyt-live-channel ' : '') . ($iscontent ? '' : ' __youtube_prefs_widget__ ') . ($isoverride ? ' epyt-is-override ' : '') . $disptypeif . ' no-lazyload"' .
                    $voloutput . $acctitle . $galleryid_ifm_data . (!empty($finalparams['streams']) ? ' data-streams="' . $finalparams['streams'] . '" ' : '') . ' allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen data-no-lazy="1" data-skipgform_ajax_framebjll=""></iframe>';
        }
...
//    }
    ...
    public static function custom_admin_pointers()
    {
        ...
        $new_pointer_content .= "This update gives guidance to YouTube\'s recent API limitations on the channel-based automatic live stream detection feature, with a solution available to Pro users.";
