...
// ie 11 doesn't do height: 100%;
                    ...
                    pageReady: function ()
                    {
                        var $channelStreams = $('.epyt-live-channel');
                        if ((window._EPYT_.not_live_on_channel || $channelStreams.length) && window._EPYT_.ytapi_load !== 'never')
                        {
                            $channelStreams.each(function ()
                            {
                                ...
                                if (!$ch.data('eypt-live-processed'))
                                {
                                    ...
                                    var allStreams = $ch.data('streams');
                                    var embedStream = null;
                                    var isActiveStream = false;
                                    if (allStreams)
                                    {
                                        var allStreamsList = JSON.parse(atob(allStreams));
                                        if (allStreamsList.length)
                                        {
                                            ...
                                            var found = false;
                                            var i = 0;
                                            while (i < allStreamsList.length)
                                            {
                                                embedStream = allStreamsList[i];
                                            ...
                                            if (found && embedStream)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        if (window._EPADashboard_.dateDiffInMinutes(now, embedStream.start) < window._EPYT_.not_live_showtime)
                                                        {
                                                            window._EPADashboard_.setVidSrc($ch, liveSrc);
                                                        }
                                                        else
